...
export class BlogService {
	...
	constructor(private contract: ContractService, private alertSvc: AlertService) {
	...
	public async executeEvent(event: BlogEvent) {
		switch (event) {
			case BlogEvent.ConnectWallet:
				let resp = await this.contract.connect();
				if (resp.success == false) this.alertSvc.alertError(resp.msg, null);
				break;
